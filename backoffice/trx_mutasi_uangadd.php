...
// Define page object
$trx_mutasi_uang_add = new ctrx_mutasi_uang_add();
$Page =& $trx_mutasi_uang_add;

// Page init processing
$trx_mutasi_uang_add->Page_Init();

// Page main processing
$trx_mutasi_uang_add->Page_Main();
?>
<?php include "header.php" ?>
<?php
	$sql="SELECT idseqno FROM trx_mutasi_uang WHERE kode LIKE '$kode%' ORDER BY idseqno DESC LIMIT 1";
...
// Create page object
var trx_mutasi_uang_add = new ew_Page("trx_mutasi_uang_add");

// page properties
trx_mutasi_uang_add.PageID = "add"; // page ID
var EW_PAGE_ID = trx_mutasi_uang_add.PageID; // for backward compatibility

// extend page with ValidateForm function
trx_mutasi_uang_add.ValidateForm = function(fobj) {
...
trx_mutasi_uang_add.Form_CustomValidate = 
...
trx_mutasi_uang_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
trx_mutasi_uang_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><!-- Add to --><h3><b>Pembiayaan dengan Kas/Bank</b></h3><br><br>
...
<?php $trx_mutasi_uang_add->ShowMessage() ?>
<form name="ftrx_mutasi_uangadd" id="ftrx_mutasi_uangadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return trx_mutasi_uang_add.ValidateForm(this);">
<p>
...
<input readonly type="text" name="x_kode" id="x_kode" size="30" maxlength="30" value="<?php echo $trx_mutasi_uang->kode->EditValue ?>"<?php echo $trx_mutasi_uang->kode->EditAttributes() ?>>
</span><?php echo $trx_mutasi_uang->kode->CustomMsg ?></td>
	</tr>
<?php } ?>
<input type="hidden" name="x_idseqno" id="x_idseqno" size="30" value="<?php echo $trx_mutasi_uang->idseqno->EditValue ?>"<?php echo $trx_mutasi_uang->idseqno->EditAttributes() ?>>
...
<input type="submit" name="btnAction" id="btnAction" value="    Simpan    ">
	...
	<script language="JavaScript">x_kode.value="<?php echo $kode; ?>";</script>
	<script language="JavaScript">x_tanggal.value="<?php echo $tanggal; ?>";</script>
	<script language="JavaScript">x_idseqno.value="<?php echo $idseqno; ?>";</script>
<script language="JavaScript" type="text/javascript">
<!--
...
<?php
$trx_mutasi_uang_add->Page_Terminate();
?>
<?php
...
class ctrx_mutasi_uang_add {
	...
	// Page Object Name
	var $PageObjName = 'trx_mutasi_uang_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $trx_mutasi_uang->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$trx_mutasi_uang->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $trx_mutasi_uang->CurrentAction = "C"; // Copy Record
		  } else {
		    $trx_mutasi_uang->CurrentAction = "I"; // Display Blank Record
		...
		switch ($trx_mutasi_uang->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$trx_mutasi_uang->SendEmail = TRUE; // Send email on add success
		...
		$trx_mutasi_uang->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $trx_mutasi_uang;
		$trx_mutasi_uang->createby->CurrentValue = $_SESSION["username"];
		$trx_mutasi_uang->createdate->CurrentValue = ew_CurrentDateTime();
	}
	...
	function RenderRow() {
		...
		} elseif ($trx_mutasi_uang->RowType == EW_ROWTYPE_ADD) { // Add row

			// kode
			$trx_mutasi_uang->kode->EditCustomAttributes = "";
			$trx_mutasi_uang->kode->EditValue = ew_HtmlEncode($trx_mutasi_uang->kode->CurrentValue);

			// idseqno
			$trx_mutasi_uang->idseqno->EditCustomAttributes = "";
			$trx_mutasi_uang->idseqno->EditValue = ew_HtmlEncode($trx_mutasi_uang->idseqno->CurrentValue);
	...
	function AddRow() {
		...
		$sFilter = $trx_mutasi_uang->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $trx_mutasi_uang->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// Field kode
		$trx_mutasi_uang->kode->SetDbValueDef($trx_mutasi_uang->kode->CurrentValue, "");
		$rsnew['kode'] =& $trx_mutasi_uang->kode->DbValue;
		...
		$bInsertRow = $trx_mutasi_uang->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($trx_mutasi_uang->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			...
			$trx_mutasi_uang->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
