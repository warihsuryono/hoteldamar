<?php
	...
	$coabank=sanitasi($_POST["coabank"]);
...
?>
							...
							<tr>
								...
								<td>
									<select name="coabank" id="coabank">
										...
										?>
											<option value="<?php echo $kode; ?>" <?php if($coabank==$kode){echo "selected";}?>><?php echo $description; ?></option>
										<?php
											}
										?>
										<option></option>
	...
	<?php
		...
		if($errmessage=="" && $load){
			...
			$bank=str_ireplace("bank","",$bank);
	?>
	<table width="100%"><tr><td align="center" <?php if($_POST["export"]) {echo "colspan=$totalcolom";} ?>><h3>MUTASI BANK <?php echo $bank; ?></h3></td></tr></table>
	<table class="content_table" <?php if($_POST["export"]) {echo "border=1";} ?>>
		...
		<?php
			$sql="SELECT sum(debit),sum(kredit) FROM trx_mutasi_uang WHERE tanggal<'$periode1' AND mode='bank' AND coabank='$coabank'";
			$hsltemp=mysql_query($sql,$db);
		...
		<?php
			$sql="SELECT tanggal,modul,kode_trx,notes,debit,kredit FROM trx_mutasi_uang WHERE tanggal BETWEEN '$periode1' AND '$periode2' AND mode='bank' AND coabank='$coabank' ORDER BY tanggal";
