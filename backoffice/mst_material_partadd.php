...
// Define page object
$mst_material_part_add = new cmst_material_part_add();
$Page =& $mst_material_part_add;

// Page init processing
$mst_material_part_add->Page_Init();

// Page main processing
$mst_material_part_add->Page_Main();
?>
...
// Create page object
var mst_material_part_add = new ew_Page("mst_material_part_add");

// page properties
mst_material_part_add.PageID = "add"; // page ID
var EW_PAGE_ID = mst_material_part_add.PageID; // for backward compatibility

// extend page with ValidateForm function
mst_material_part_add.ValidateForm = function(fobj) {
...
mst_material_part_add.Form_CustomValidate = 
...
mst_material_part_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mst_material_part_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><!-- Add to --><h3><b>Master Barang</b></h3><br><br>
...
<?php $mst_material_part_add->ShowMessage() ?>
<form name="fmst_material_partadd" id="fmst_material_partadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mst_material_part_add.ValidateForm(this);">
<p>
...
<?php
$mst_material_part_add->Page_Terminate();
?>
<?php
...
class cmst_material_part_add {
	...
	// Page Object Name
	var $PageObjName = 'mst_material_part_add';
	...
	//  - open connection
	//
	function cmst_material_part_add() {
		global $conn;
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $mst_material_part->CurrentAction = $_POST["a_add"]; // Get form action
		  ...
		  if ($bCopy) {
		    $mst_material_part->CurrentAction = "C"; // Copy Record
		  } else {
		    $mst_material_part->CurrentAction = "I"; // Display Blank Record
		...
		switch ($mst_material_part->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$mst_material_part->SendEmail = TRUE; // Send email on add success
		...
		$mst_material_part->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function RenderRow() {
		...
		} elseif ($mst_material_part->RowType == EW_ROWTYPE_ADD) { // Add row

			// kode
			$mst_material_part->kode->EditCustomAttributes = "";
			$mst_material_part->kode->EditValue = ew_HtmlEncode($mst_material_part->kode->CurrentValue);
	...
	function AddRow() {
		...
		$sFilter = $mst_material_part->KeyFilter();
		if ($bCheckKey) {
			$rsChk = $mst_material_part->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
		...
		// Field kode
		$mst_material_part->kode->SetDbValueDef($mst_material_part->kode->CurrentValue, "");
		$rsnew['kode'] =& $mst_material_part->kode->DbValue;
		...
		$bInsertRow = $mst_material_part->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($mst_material_part->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			...
			$mst_material_part->Row_Inserted($rsnew);
		}
		return $AddRow;
	}
