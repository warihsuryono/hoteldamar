<?php
	...
	if($modebutton=="simpan"){
		...
		$sql="SELECT koder FROM acc_mst_coa WHERE coa='$coabank'";
		$hsltemp=mysql_query($sql,$db);
		list($koder)=mysql_fetch_array($hsltemp);
		$keterangan="Pembiayaan dengan Bank";
		$sql="INSERT INTO acc_jurnal_detail (kodejurnal,seqno,coa,koder,keterangan,debit,kredit) VALUES ('$kodejurnal','$seqno','$coabank','$koder','$keterangan','$debit','$kredit')";
		mysql_query($sql,$db);
		?>
...
?>
	<form method="POST" action="<?php echo $__phpself; ?>?editing=<?php echo $_GET["editing"];?>&kode=<?php echo $_GET["kode"]; ?>">
		...
		<table width="100%"><tr><td align="center"><h3><b>PEMBIAYAAN DENGAN BANK</b></h3></td></tr></table>
		<table>
			<tr>
				<td>Kode Jurnal</td>
				<td>:</td>
...
<?php
	if(!$_GET["editing"]){//cari kodejurnal [MR/M/yyyymmdd/xxx]
		...
		?><script language="javascript">document.getElementById("posting").value="bank";</script><?php
