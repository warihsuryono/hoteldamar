<?php
	...
	if($_GET["editing"]){
		$sql="SELECT ambilby FROM $tablename WHERE $kodename='".$_GET["kode"]."'";
		$hsltemp=mysql_query($sql,$db);
	...
	if($modebutton=="simpan"){
		...
		$beriby=$__username;
		$beridate=$__now;
		$detailfile=$tabledetailname.".php?kode=$transkode";
		$actionlink="<a href=''$__phpself?editing=1&kode=$transkode''><img src=''images/inlineedit.gif'' title=''Inline Edit'' width=''16'' height=''16'' border=''0''></a>";
	...
	<form method="POST" action="<?php echo $__phpself; ?>?editing=<?php echo $_GET["editing"];?>&kode=<?php echo $_GET["kode"]; ?>">
		...
		<table>
			<tr>
				...
				<td>:</td>
				<td><input type="text" id="transkode" name="transkode" readonly size="30"></td>
			</tr>
			<!--tr>
			...
			<tr>
				...
				<td>
					...
					<!--img src="images/b_search.png" title="Daftar Purchase Order" border="0" width="13" height="13" onclick="showPO('pono',kode_pekerjaan.value,'')"-->
					<img src="images/b_search.png" title="Daftar Purchase Order" border="0" width="13" height="13" onclick="showPO('pono','','')">
				</td>
			</tr>
			<tr>
		...
		<table class="content_table" width="100%" id="tabledetail">
			...
			<tr id="rowdetail" class="content_ganjil">
				...
				<!--td><input id="projectqty[0]" type="text" name="projectqty[0]" size="5" style="text-align:right;" readonly></td>
				<td><input id="outstandingqty[0]" type="text" name="outstandingqty[0]" size="5" style="text-align:right;" readonly></td-->
				<td><input id="transqty[0]" type="text" name="transqty[0]" size="5" style="text-align:right;"></td>
				<td>
...
<?php
	if(!$_GET["editing"]){//cari mrno [TRAN/M/yyyymmdd/xxx]
		$tanggal=date("Y-m-d");
		$kode="TRAN/M/".date("Ymd")."/";
		...
		$periode=date("Y-m-01");
		$sql="SELECT gudang FROM user_account WHERE username='$__username'";
		$hsltemp=mysql_query($sql,$db);
		...
		?><script language="javascript">
			//document.getElementById("gudang").value="<?php echo $gudang; ?>";
		</script><?php
	}else{//load editing
		$kode=$_GET["kode"];
		$sql="SHOW COLUMNS FROM $tablename WHERE field<>'xtimestamp'";
