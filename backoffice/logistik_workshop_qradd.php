	...
	<form method="POST" action="<?php echo $__phpself; ?>?editing=<?php echo $_GET["editing"];?>&kode=<?php echo $_GET["kode"]; ?>">
		...
		<table>
			...
			<tr>
				...
				<td>:</td>
				<td>
					<input type="text" id="prno" name="prno" readonly size="30">
					<img src="images/b_search.png" title="Daftar Permintaan Part" border="0" width="13" height="13" onclick="showPR('prno')">
			...
			<tr>
				...
				<td>:</td>
				<td>
					<select name="partunit" id="partunit">
	...
	<table width="100%">
		<tr>
			<td align="center">
				...
				<?php if($_GET["editing"]){ ?>
				<input type="button" value="Daftar Penerimaan Quotation" onclick="window.open('logistik_workshop_qr_vendorlist.php?qrno='+qrno.value,'logistik_workshop_qr_vendorlist','width=1000,height=800,scrollbars=yes');">
				<input type="button" value="Terima Quotation" onclick="window.open('logistik_workshop_qr_vendoradd.php?qrno='+qrno.value,'logistik_workshop_qr_vendoradd','width=1000,height=800,scrollbars=yes');">
				<?php } ?>
			</td>
		</tr>
	</table>
<?php
	if(!$_GET["editing"]){//cari qrno [QR/P/yyyymmdd/xxx]
		$tanggal=date("Y-m-d");
		$kode="QR/P/".date("Ymd")."/";
