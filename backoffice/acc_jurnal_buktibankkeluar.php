<?php
	...
	$tablename=str_ireplace("_buktibankkeluar.php","",basename($__phpself));
...
?>
	<form method="POST" action="<?php echo $__phpself; ?>?editing=<?php echo $_GET["editing"];?>&kode=<?php echo $_GET["kode"]; ?>">
		...
		<?php $__captiontitle="BUKTI BANK KELUAR";include_once "header_document.php"; ?>
		<table width="100%">
			<tr>
				<td valign="top">
					<table>
...
<?php
	...
	$sql="SELECT coa FROM $tabledetailname WHERE $kodename='$kode' AND debit=0 LIMIT 1";
	$hsltemp=mysql_query($sql,$db);
	...
	$sql="SELECT $selectdetail FROM $tabledetailname WHERE $kodename='$kode' AND kredit=0";
