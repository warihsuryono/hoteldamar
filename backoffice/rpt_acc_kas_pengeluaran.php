			...
			<table>
				...
				<tr>
					<td colspan="3">
						...
						<input type="button" name="detail" value="Load Detail" onclick="window.open('rpt_acc_kas_pengeluaran_detail.php?arrpost=<?php echo $arrpost; ?>','rpt_acc_kas_pengeluaran_detail','width=800,height=600,top=300,scrollbars=yes');">
...
<table width="100%" class="content_table" <?php if($_POST["export"]) {echo "border=1";} ?>>
	...
	<?php
		...
		$sql="SELECT kodejurnal,kredit FROM acc_jurnal_detail WHERE kodejurnal IN (SELECT kodejurnal FROM acc_jurnal WHERE tanggal LIKE '$_tanggal%') AND coa='$coakas' AND debit=0 AND kredit <> 0";
		$hsltemp=mysql_query($sql,$db);
		$debitkas=0;
	...
	<?php
		...
		while(list($coabank)=mysql_fetch_array($hslbank)){
			...
			$hsltemp=mysql_query($sql,$db);
			list($keterangan)=mysql_fetch_array($hsltemp);
			$sql="SELECT kodejurnal,kredit FROM acc_jurnal_detail WHERE kodejurnal IN (SELECT kodejurnal FROM acc_jurnal WHERE tanggal LIKE '$_tanggal%') AND coa='$coabank' AND debit=0 AND kredit <> 0";
			$hsltemp=mysql_query($sql,$db);
			//echo "<br>$sql";
			$debitbank=0;
			while(list($kodejurnal,$debit)=mysql_fetch_array($hsltemp)){
	...
	<?php
		$kredittotal=0;
		$sql="SELECT coa,sum(debit) FROM acc_jurnal_detail WHERE kodejurnal IN ($_kodejurnal) AND kredit='0' GROUP BY coa";
