...
// Define page object
$mst_room_price_edit = new cmst_room_price_edit();
$Page =& $mst_room_price_edit;

// Page init processing
$mst_room_price_edit->Page_Init();

// Page main processing
$mst_room_price_edit->Page_Main();
?>
...
// Create page object
var mst_room_price_edit = new ew_Page("mst_room_price_edit");

// page properties
mst_room_price_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = mst_room_price_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
mst_room_price_edit.ValidateForm = function(fobj) {
...
mst_room_price_edit.Form_CustomValidate = 
...
mst_room_price_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mst_room_price_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><!-- Edit --> <h3><b>Master Room Price</b></h3><br><br>
...
<?php $mst_room_price_edit->ShowMessage() ?>
<form name="fmst_room_priceedit" id="fmst_room_priceedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mst_room_price_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="mst_room_price">
...
<?php if ($mst_room_price->id->Visible) { // id ?>
	<tr<?php echo $mst_room_price->id->RowAttributes ?>>
		...
		<td<?php echo $mst_room_price->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $mst_room_price->id->ViewAttributes() ?>><?php echo $mst_room_price->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($mst_room_price->id->CurrentValue) ?>">
</span><?php echo $mst_room_price->id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if($mst_room_price->id->EditValue>2) { ?>
...
<?php
$mst_room_price_edit->Page_Terminate();
?>
<?php
...
class cmst_room_price_edit {
	...
	// Page Object Name
	var $PageObjName = 'mst_room_price_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$mst_room_price->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$mst_room_price->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$mst_room_price->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($mst_room_price->id->CurrentValue == "")
			$this->Page_Terminate("mst_room_pricelist.php"); // Invalid key, return to list
		switch ($mst_room_price->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$mst_room_price->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$mst_room_price->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($mst_room_price->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$mst_room_price->id->EditCustomAttributes = "";
			$mst_room_price->id->EditValue = $mst_room_price->id->CurrentValue;
			$mst_room_price->id->CssStyle = "";
			$mst_room_price->id->CssClass = "";
			$mst_room_price->id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $mst_room_price->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($mst_room_price->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$mst_room_price->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
