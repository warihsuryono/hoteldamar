...
// Define page object
$logistik_qr_search = new clogistik_qr_search();
$Page =& $logistik_qr_search;

// Page init processing
$logistik_qr_search->Page_Init();

// Page main processing
$logistik_qr_search->Page_Main();
?>
...
// Create page object
var logistik_qr_search = new ew_Page("logistik_qr_search");

// page properties
logistik_qr_search.PageID = "search"; // page ID
var EW_PAGE_ID = logistik_qr_search.PageID; // for backward compatibility

// extend page with validate function for search
logistik_qr_search.ValidateSearch = function(fobj) {
...
logistik_qr_search.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
logistik_qr_search.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
logistik_qr_search.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
logistik_qr_search.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
logistik_qr_search.ShowHighlightText = "Show highlight"; 
logistik_qr_search.HideHighlightText = "Hide highlight";
...
<p><span class="phpmaker">Search <h3><b>Permintaan Quotation</b></h3><br><br>
<!--a href="<?php echo $logistik_qr->getReturnUrl() ?>">Back to List</a-->
<input type="button" value="Kembali" onclick="window.location='<?php echo $logistik_qr->getReturnUrl() ?>';">
</span></p>
<?php $logistik_qr_search->ShowMessage() ?>
<form name="flogistik_qrsearch" id="flogistik_qrsearch" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return logistik_qr_search.ValidateSearch(this);">
<p>
<input type="hidden" name="t" id="t" value="logistik_qr">
<input type="hidden" name="a_search" id="a_search" value="S">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table cellspacing="0" class="ewTable">
	<tr<?php echo $logistik_qr->actionlink->RowAttributes ?>>
		<td class="ewTableHeader"></td>
		<td<?php echo $logistik_qr->actionlink->CellAttributes() ?>><span class="ewSearchOpr">contains<input type="hidden" name="z_actionlink" id="z_actionlink" value="LIKE"></span></td>
		<td<?php echo $logistik_qr->actionlink->CellAttributes() ?>>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="phpmaker">
<textarea name="x_actionlink" id="x_actionlink" cols="35" rows="4"<?php echo $logistik_qr->actionlink->EditAttributes() ?>><?php echo $logistik_qr->actionlink->EditValue ?></textarea>
</span></td>
			</tr></table>
		</td>
	</tr>
...
<?php
$logistik_qr_search->Page_Terminate();
?>
<?php
...
class clogistik_qr_search {
	...
	// Page Object Name
	var $PageObjName = 'logistik_qr_search';
	...
	//
	function Page_Main() {
		global $objForm, $gsSearchError, $logistik_qr;
		$objForm = new cFormObj();
		if ($this->IsPageRequest()) { // Validate request

			// Get action
			$logistik_qr->CurrentAction = $objForm->GetValue("a_search");
			switch ($logistik_qr->CurrentAction) {
				case "S": // Get Search Criteria
					...
					if ($sSrchStr <> "") {
						$sSrchStr = $logistik_qr->UrlParm($sSrchStr);
						$this->Page_Terminate("logistik_qrlist.php" . "?" . $sSrchStr); // Go to list page
					}
			}
		}
...
function BuildAdvancedSearch() {
	global $logistik_qr;
	$sSrchUrl = "";
	$this->BuildSearchUrl($sSrchUrl, $logistik_qr->actionlink); // actionlink
	return $sSrchUrl;
}
...
function BuildSearchUrl(&$Url, &$Fld) {
	...
	function LoadSearchValues() {
		...
		$logistik_qr->actionlink->AdvancedSearch->SearchValue = $objForm->GetValue("x_actionlink");
		$logistik_qr->actionlink->AdvancedSearch->SearchOperator = $objForm->GetValue("z_actionlink");

		// qrno
		$logistik_qr->qrno->AdvancedSearch->SearchValue = $objForm->GetValue("x_qrno");
		$logistik_qr->qrno->AdvancedSearch->SearchOperator = $objForm->GetValue("z_qrno");

		// kode_pekerjaan
		$logistik_qr->kode_pekerjaan->AdvancedSearch->SearchValue = $objForm->GetValue("x_kode_pekerjaan");
		$logistik_qr->kode_pekerjaan->AdvancedSearch->SearchOperator = $objForm->GetValue("z_kode_pekerjaan");

		// tanggal
		$logistik_qr->tanggal->AdvancedSearch->SearchValue = $objForm->GetValue("x_tanggal");
		$logistik_qr->tanggal->AdvancedSearch->SearchOperator = $objForm->GetValue("z_tanggal");

		// periode
		$logistik_qr->periode->AdvancedSearch->SearchValue = $objForm->GetValue("x_periode");
		$logistik_qr->periode->AdvancedSearch->SearchOperator = $objForm->GetValue("z_periode");

		// gudang
		$logistik_qr->gudang->AdvancedSearch->SearchValue = $objForm->GetValue("x_gudang");
		$logistik_qr->gudang->AdvancedSearch->SearchOperator = $objForm->GetValue("z_gudang");

		// createby
		$logistik_qr->createby->AdvancedSearch->SearchValue = $objForm->GetValue("x_createby");
		$logistik_qr->createby->AdvancedSearch->SearchOperator = $objForm->GetValue("z_createby");
	}
	...
	function RenderRow() {
		...
		$logistik_qr->actionlink->CellCssStyle = "";
	...
	function LoadAdvancedSearch() {
		global $logistik_qr;
		$logistik_qr->actionlink->AdvancedSearch->SearchValue = $logistik_qr->getAdvancedSearch("x_actionlink");
