...
<table width="100%" class="content_table" <?php if($_POST["export"]) {echo "border=1";} ?>>
	...
	<?php
		...
		$sql="SELECT kodejurnal FROM acc_jurnal_detail WHERE kodejurnal IN (SELECT kodejurnal FROM acc_jurnal WHERE tanggal LIKE '$_tanggal%') AND coa='$coakas' AND kredit=0";
		$hsltemp=mysql_query($sql,$db);
		...
		while(list($coabank)=mysql_fetch_array($hslbank)){
			$sql="SELECT kodejurnal FROM acc_jurnal_detail WHERE kodejurnal IN (SELECT kodejurnal FROM acc_jurnal WHERE tanggal LIKE '$_tanggal%') AND coa='$coabank' AND kredit=0";
			$hsltemp=mysql_query($sql,$db);
			$debitbank=0;
			while(list($kodejurnal)=mysql_fetch_array($hsltemp)){
				$arrkodejurnal[$kodejurnal]=1;
		...
		$sql="SELECT kodejurnal,coa,keterangan,kredit FROM acc_jurnal_detail WHERE kodejurnal IN ($_kodejurnal) AND debit='0' ORDER BY kodejurnal";
