...
// Define page object
$mst_menuweb_edit = new cmst_menuweb_edit();
$Page =& $mst_menuweb_edit;

// Page init processing
$mst_menuweb_edit->Page_Init();

// Page main processing
$mst_menuweb_edit->Page_Main();
?>
...
// Create page object
var mst_menuweb_edit = new ew_Page("mst_menuweb_edit");

// page properties
mst_menuweb_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = mst_menuweb_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
mst_menuweb_edit.ValidateForm = function(fobj) {
...
mst_menuweb_edit.Form_CustomValidate = 
...
mst_menuweb_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mst_menuweb_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><!-- Edit --> <h3><b>Web Menu</b></h3><br><br>
...
<?php $mst_menuweb_edit->ShowMessage() ?>
<form name="fmst_menuwebedit" id="fmst_menuwebedit" action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="a_table" id="a_table" value="mst_menuweb">
...
<?php if ($mst_menuweb->kode->Visible) { // kode ?>
	<tr<?php echo $mst_menuweb->kode->RowAttributes ?>>
		...
		<td<?php echo $mst_menuweb->kode->CellAttributes() ?>><span id="el_kode">
<div<?php echo $mst_menuweb->kode->ViewAttributes() ?>><?php echo $mst_menuweb->kode->EditValue ?></div><input type="hidden" name="x_kode" id="x_kode" value="<?php echo ew_HtmlEncode($mst_menuweb->kode->CurrentValue) ?>">
</span><?php echo $mst_menuweb->kode->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<p>
<!--input type="button" name="btnAction" id="btnAction" value="   Edit   " onclick="ew_SubmitForm(mst_menuweb_edit, this.form);" /-->
<input type="button" name="btnAction" id="btnAction" value="    Simpan    " onclick="ew_SubmitForm(mst_menuweb_edit, this.form);">
</form>
<script type="text/javascript">
<!--
...
<?php
$mst_menuweb_edit->Page_Terminate();
?>
<?php
...
class cmst_menuweb_edit {
	...
	// Page Object Name
	var $PageObjName = 'mst_menuweb_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$mst_menuweb->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$mst_menuweb->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$mst_menuweb->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($mst_menuweb->kode->CurrentValue == "")
			$this->Page_Terminate("mst_menuweblist.php"); // Invalid key, return to list
		switch ($mst_menuweb->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$mst_menuweb->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$mst_menuweb->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($mst_menuweb->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// kode
			$mst_menuweb->kode->EditCustomAttributes = "";
			$mst_menuweb->kode->EditValue = $mst_menuweb->kode->CurrentValue;
			$mst_menuweb->kode->CssStyle = "";
			$mst_menuweb->kode->CssClass = "";
			$mst_menuweb->kode->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $mst_menuweb->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($mst_menuweb->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$mst_menuweb->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
