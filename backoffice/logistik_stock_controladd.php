<?php
	...
	if($modebutton=="simpan"){
		...
		$cekby=$__username;
		$cekdate=$__now;
		$detailfile=$tabledetailname.".php?kode=$kodecek";
		$actionlink="<a href=''$__phpself?editing=1&kode=$kodecek''><img src=''images/inlineedit.gif'' title=''Inline Edit'' width=''16'' height=''16'' border=''0''></a>";
	...
	<form method="POST" action="<?php echo $__phpself; ?>?editing=<?php echo $_GET["editing"];?>&kode=<?php echo $_GET["kode"]; ?>">
		...
		<table>
			<tr>
				<td>Kode Kontrol</td>
				<td>:</td>
				<td><input type="text" id="kodecek" name="kodecek" readonly size="30"></td>
			</tr>
			<!--tr>
				<td>Gudang</td>
				<td>:</td>
		...
		<table class="content_table" width="100%" id="tabledetail">
			...
			<tr id="rowdetail" class="content_ganjil">
				...
				<td><input id="systemqty[0]" type="text" name="systemqty[0]" size="5" style="text-align:right;" readonly></td>
				<td><input id="realqty[0]" type="text" name="realqty[0]" size="5" style="text-align:right;"></td>
				<td>
...
<?php
	if(!$_GET["editing"]){//cari kodecek [CTR/M/yyyymmdd/xxx]
		$tanggal=date("Y-m-d");
		$kode="CTR/M/".date("Ymd")."/";
