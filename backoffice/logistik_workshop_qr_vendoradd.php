<?php
	...
	$hsltemp=mysql_query($sql,$db);
	for($xx=0;$xx<4;$xx++){list($kodedetailname)=mysql_fetch_array($hsltemp);}//partno
...
?>
	...
	<script language="javascript">
		function checkqrvendor(qrno,vendorid){
			...
			xmlHttp.open("GET","ajax.checkqrvendor.php?mode=workshop&qrno="+qrno+"&vendorid="+vendorid,true);
			xmlHttp.send(null);	
		}
...
<?php
	if(!$_GET["editing"]){//cari qrno [QR/M/yyyymmdd/xxx]
		$tanggal=date("Y-m-d");
		$kode=$_GET["qrno"];
		$sql="SELECT kode_pekerjaan,tanggal,periode FROM logistik_workshop_qr WHERE qrno='$kode'";
		$hsltemp=mysql_query($sql,$db);
		...
		//load detail qr
		$sql="SELECT count(partno) FROM logistik_workshop_qr_detail WHERE qrno='$kode'";
		$hsltemp=mysql_query($sql,$db);
		...
		$sql="SELECT partno,seqno,qty,satuan,keterangan FROM logistik_workshop_qr_detail WHERE qrno='$kode'";
		$hsldetqr=mysql_query($sql,$db);
		while(list($partno,$seqno,$qty,$satuan,$keterangan)=mysql_fetch_array($hsldetqr)){
			?><script language="javascript">document.getElementById("partno[<?php echo $seqno; ?>]").value="<?php echo $partno; ?>";</script><?php
		...
		while($detailvalues=mysql_fetch_assoc($hsldet)){
			...
			foreach ($detailvalues as $vardetailname => $valuedetail){
				...
				//echo "<br>document.getElementById('$vardetailname [$no]').value=$valuedetail";
			}
			$sql="SELECT nama FROM mst_material_part WHERE kode='$partno'";
			$hsltemp=mysql_query($sql,$db);
			list($namabarang)=mysql_fetch_array($hsltemp);
