...
// Define page object
$mst_vendor_edit = new cmst_vendor_edit();
$Page =& $mst_vendor_edit;

// Page init processing
$mst_vendor_edit->Page_Init();

// Page main processing
$mst_vendor_edit->Page_Main();
?>
...
// Create page object
var mst_vendor_edit = new ew_Page("mst_vendor_edit");

// page properties
mst_vendor_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = mst_vendor_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
mst_vendor_edit.ValidateForm = function(fobj) {
...
mst_vendor_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
mst_vendor_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
mst_vendor_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mst_vendor_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// search highlight properties
mst_vendor_edit.ShowHighlightText = "Show highlight"; 
mst_vendor_edit.HideHighlightText = "Hide highlight";
...
<p><span class="phpmaker"><!-- Edit --> <h3><b>Master Supplier</b></h3><br><br>
...
<?php $mst_vendor_edit->ShowMessage() ?>
<form name="fmst_vendoredit" id="fmst_vendoredit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mst_vendor_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="mst_vendor">
...
<div<?php echo $mst_vendor->kode->ViewAttributes() ?>><?php echo $mst_vendor->kode->EditValue ?></div><input type="hidden" name="x_kode" id="x_kode" value="<?php echo ew_HtmlEncode($mst_vendor->kode->CurrentValue) ?>">
...
<?php
$mst_vendor_edit->Page_Terminate();
?>
<?php
...
class cmst_vendor_edit {
	...
	// Page Object Name
	var $PageObjName = 'mst_vendor_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$mst_vendor->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$mst_vendor->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$mst_vendor->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($mst_vendor->kode->CurrentValue == "")
			$this->Page_Terminate("mst_vendorlist.php"); // Invalid key, return to list
		switch ($mst_vendor->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$mst_vendor->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$mst_vendor->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
