...
// PHPMaker 6 configuration for Table logistik_po
$logistik_po = NULL; // Initialize table object
...
class clogistik_po {
	...
	function clogistik_po() {
		$this->TableVar = "logistik_po";
		$this->TableName = "logistik_po";
		$this->SelectLimit = TRUE;
		$this->actionlink = new cField('logistik_po', 'x_actionlink', 'actionlink', "`actionlink`", 201, -1, FALSE);
		$this->fields['actionlink'] =& $this->actionlink;
		$this->pono = new cField('logistik_po', 'x_pono', 'pono', "`pono`", 200, -1, FALSE);
		$this->fields['pono'] =& $this->pono;
		$this->idseqno = new cField('logistik_po', 'x_idseqno', 'idseqno', "`idseqno`", 3, -1, FALSE);
		$this->fields['idseqno'] =& $this->idseqno;
		$this->tanggal = new cField('logistik_po', 'x_tanggal', 'tanggal', "`tanggal`", 133, 7, FALSE);
		$this->fields['tanggal'] =& $this->tanggal;
		$this->kode_pekerjaan = new cField('logistik_po', 'x_kode_pekerjaan', 'kode_pekerjaan', "`kode_pekerjaan`", 200, -1, FALSE);
		$this->fields['kode_pekerjaan'] =& $this->kode_pekerjaan;
		$this->kodepermohonan = new cField('logistik_po', 'x_kodepermohonan', 'kodepermohonan', "`kodepermohonan`", 200, -1, FALSE);
		$this->fields['kodepermohonan'] =& $this->kodepermohonan;
		$this->vendorid = new cField('logistik_po', 'x_vendorid', 'vendorid', "`vendorid`", 200, -1, FALSE);
		$this->fields['vendorid'] =& $this->vendorid;
		$this->qrno = new cField('logistik_po', 'x_qrno', 'qrno', "`qrno`", 200, -1, FALSE);
		$this->fields['qrno'] =& $this->qrno;
		$this->createby = new cField('logistik_po', 'x_createby', 'createby', "`createby`", 200, -1, FALSE);
		$this->fields['createby'] =& $this->createby;
		$this->createdate = new cField('logistik_po', 'x_createdate', 'createdate', "`createdate`", 135, 7, FALSE);
		$this->fields['createdate'] =& $this->createdate;
		$this->tahuby = new cField('logistik_po', 'x_tahuby', 'tahuby', "`tahuby`", 200, -1, FALSE);
		$this->fields['tahuby'] =& $this->tahuby;
		$this->tahudate = new cField('logistik_po', 'x_tahudate', 'tahudate', "`tahudate`", 135, 7, FALSE);
		$this->fields['tahudate'] =& $this->tahudate;
		$this->setujuby = new cField('logistik_po', 'x_setujuby', 'setujuby', "`setujuby`", 200, -1, FALSE);
		$this->fields['setujuby'] =& $this->setujuby;
		$this->setujudate = new cField('logistik_po', 'x_setujudate', 'setujudate', "`setujudate`", 135, 7, FALSE);
	...
	function SqlSelect() { // Select
		return "SELECT * FROM `logistik_po`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `logistik_po` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE `logistik_po` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `logistik_po` WHERE ";
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("logistik_poview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "logistik_poadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("logistik_poedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("logistik_poadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("logistik_podelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=logistik_po" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
