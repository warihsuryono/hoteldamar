...
// PHPMaker 6 configuration for Table logistik_qr_vendor
$logistik_qr_vendor = NULL; // Initialize table object

// Define table class
class clogistik_qr_vendor {
	...
	function clogistik_qr_vendor() {
		$this->TableVar = "logistik_qr_vendor";
		$this->TableName = "logistik_qr_vendor";
		$this->SelectLimit = TRUE;
		$this->actionlink = new cField('logistik_qr_vendor', 'x_actionlink', 'actionlink', "`actionlink`", 201, -1, FALSE);
		$this->fields['actionlink'] =& $this->actionlink;
		$this->qrno = new cField('logistik_qr_vendor', 'x_qrno', 'qrno', "`qrno`", 200, -1, FALSE);
		$this->fields['qrno'] =& $this->qrno;
		$this->tanggal = new cField('logistik_qr_vendor', 'x_tanggal', 'tanggal', "`tanggal`", 133, 7, FALSE);
		$this->fields['tanggal'] =& $this->tanggal;
		$this->seqno = new cField('logistik_qr_vendor', 'x_seqno', 'seqno', "`seqno`", 3, -1, FALSE);
		$this->fields['seqno'] =& $this->seqno;
		$this->vendorid = new cField('logistik_qr_vendor', 'x_vendorid', 'vendorid', "`vendorid`", 200, -1, FALSE);
		$this->fields['vendorid'] =& $this->vendorid;
		$this->notes = new cField('logistik_qr_vendor', 'x_notes', 'notes', "`notes`", 201, -1, FALSE);
		$this->fields['notes'] =& $this->notes;
		$this->receiveby = new cField('logistik_qr_vendor', 'x_receiveby', 'receiveby', "`receiveby`", 200, -1, FALSE);
		$this->fields['receiveby'] =& $this->receiveby;
		$this->receivedate = new cField('logistik_qr_vendor', 'x_receivedate', 'receivedate', "`receivedate`", 135, 7, FALSE);
	...
	function HighlightName() {
		return "logistik_qr_vendor_Highlight";
	}
	...
	function SqlSelect() { // Select
		return "SELECT * FROM `logistik_qr_vendor`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `logistik_qr_vendor` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE `logistik_qr_vendor` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `logistik_qr_vendor` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[EW_PROJECT_NAME . "_" . $this->TableVar . "_" . EW_TABLE_RETURN_URL] <> "") {
			...
			return "logistik_qr_vendorlist.php";
		}
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("logistik_qr_vendorview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "logistik_qr_vendoradd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("logistik_qr_vendoredit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("logistik_qr_vendoradd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("logistik_qr_vendordelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=logistik_qr_vendor" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
