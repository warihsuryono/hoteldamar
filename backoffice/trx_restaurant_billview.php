<?php
	...
	if($_GET["read"]){
		$kode=$_GET["kode"];
		$sql="UPDATE trx_restaurant_bill SET isread='1' WHERE kode='".$_GET["kode"]."'";
		mysql_query($sql,$db);
		?>
	...
	if($_GET["paid"]){
		...
		$sql="UPDATE trx_restaurant_bill SET paid='1',paymenttype='$paymenttype',coabank='$coabank',norek='$norek' WHERE kode='".$_GET["kode"]."'";
		mysql_query($sql,$db);
		//INSERT ACCOUNTING
		if(!$tanggalbayar){$tanggal=date("Y-m-d");}else{$tanggal=$tanggalbayar;}
		
		$createby=$__username;
		$sql="SELECT room FROM trx_restaurant_bill WHERE kode='".$_GET["kode"]."'";
		$hsltemp=mysql_query($sql,$db);
		list($room)=mysql_fetch_array($hsltemp);
		$sql="SELECT nama FROM mst_room WHERE kode='$room'";
		$hsltemp=mysql_query($sql,$db);
		list($room)=mysql_fetch_array($hsltemp);
		$notes="Restaurant Bill Room $room";
		
		$kodejurnal=add_jurnal($tanggal,$norek,$vendor,$notes);
		
		$sql="SELECT notes,room,nama,withppn,withservice,nett,disc,discname FROM trx_restaurant_bill WHERE kode='".$_GET["kode"]."'";
		$hsltemp=mysql_query($sql,$db);
		list($notes,$room,$namacust,$withtax,$withservice,$_nettrest,$_disc,$_discname)=mysql_fetch_array($hsltemp);
		$sql="SELECT nama FROM mst_room WHERE kode='$room'";
		$hsltemp=mysql_query($sql,$db);
		list($room)=mysql_fetch_array($hsltemp);
		...
		$sql="SELECT foodid,qty,price,keterangan FROM trx_restaurant_bill_detail WHERE kode='$kode'";
		...
		if($withtax || true){
			...
			$keterangan="Pajak Masukan Restaurant";
		...
		if($_GET["paidvoucher"]){
			...
			$hsltemp=mysql_query($sql,$db);
			list($coadisc)=mysql_fetch_array($hsltemp);
			$keterangan="Discount Voucher Restaurant [$room]";
			if($notes){$keterangan.=" ($notes)";}
			$debit=0;
		...
		//KAS/BANK (Debit)
		$keterangan="Restaurant Bill [$room]";
		if($notes){$keterangan.=" ($notes)";}
		$kredit=0;
		...
		if($paycash>0){
			...
			$hsltemp=mysql_query($sql,$db);
			list($coa1)=mysql_fetch_array($hsltemp);
			$keterangan="Restaurant Bill Cash [$room] ($namacust)";
		...
		if($payedc1>0){
			$keterangan="Restaurant Bill EDC [$room] ($namacust/$noedc1)";
		...
		if($payedc2>0){
			$keterangan="Restaurant Bill EDC [$room] ($namacust/$noedc2)";
		...
		if($paytrf>0){
			$keterangan="Restaurant Bill Transfer [$room] ($namacust/$notrf)";
		...
		$notes="Restaurant Bill [$room] ($namacust)";
		$sql="UPDATE acc_jurnal SET notes='$notes' WHERE kodejurnal='$kodejurnal';";
		mysql_query($sql,$db);
	}
...
?>
	...
	<form method="POST" action="<?php echo $__phpself; ?>?editing=<?php echo $_GET["editing"];?>&kode=<?php echo $_GET["kode"]; ?>">
		...
		<?php $__captiontitle="RESTAURANT BILL";include_once "header_document.php"; ?>
		<table>
			<tr>
		...
		<table class="content_table" width="100%" id="tabledetail">
			...
			<tr id="rowdetail" class="content_ganjil">
				<td align="right">  1 </td>
				<!--td>
					<input id="foodid[0]" type="text" name="foodid[0]" size="20" readonly>
				</td-->
				<td id="namamakanan[0]"></td>
			...
			<?php
				$sql="SELECT withppn,withservice,subtotal1,ppn,subtotal2,service,disc,grandtotal,paid,isread FROM trx_restaurant_bill WHERE kode='".$_GET["kode"]."'";
				$hsltemp=mysql_query($sql,$db);
				list($_withppn,$_withservice,$_subtotal1,$_ppn,$_subtotal2,$_service,$_disc,$_grandtotal,$_paid,$_isread)=mysql_fetch_array($hsltemp);
	...
	<table width="100%">
		<tr>
			<td align="center">
				...
				<?php if(!$_isread){ ?>
					<input type="button" value="Read" id="readbutton" onclick="window.location='<?php echo $__phpself;?>?read=1&kode=<?php echo $_GET["kode"]; ?>';">
				<?php }else{ ?>
					READ!
				<?php } ?>
...
<?php
		...
		while($detailvalues=mysql_fetch_assoc($hsldet)){
			...
			?><script language="javascript">document.getElementById("namamakanan[<?php echo $no; ?>]").innerHTML="<?php echo $namamakanan; ?>";</script><?php
