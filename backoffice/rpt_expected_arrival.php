...
?>
	...
	<table width="100%"><tr><td align="center" <?php if($_POST["export"]) {echo "colspan=$totalcolom";} ?>><h3>EXPECTED ARRIVAL</h3></td></tr></table>
	...
	<?php
		$sql="SELECT kode,title,nama,idtype,idno,phone,room,departure,checkin,checkindate FROM trx_booking WHERE arrival = '".$periode1."' ORDER BY room";
	...
	?>
		...
		<table class="content_table" <?php if($_POST["export"]) {echo "border=1";} ?>>
			...
			<?php
				$no=0;
				while(list($kode,$title,$nama,$idtype,$idno,$phone,$room,$departure,$checkin,$checkindate)=mysql_fetch_array($hslbook)){
					$no++;
			...
			?>
				<tr>
					...
					<td><?=$idtype;?> <?php echo $idno; ?></td>
					<td><?php echo $phone; ?></td>
					<td><?php echo format_tanggal3($departure); ?></td>
					<td>
						<?php if(!$checkin){ ?>
						<input type="button" value="Check IN" onclick="window.location='trx_bookingview.php?kode=<?=$kode;?>&checkoutDate=<?=$periode1;?>';">
						<?php } else { echo "<b>Checked in at ".format_tanggal3($checkindate)."</b>"; } ?>
					</td>
				</tr>
			<?php
				}
