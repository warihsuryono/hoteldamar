...
// PHPMaker 6 configuration for Table logistik_workshop_qr
$logistik_workshop_qr = NULL; // Initialize table object

// Define table class
class clogistik_workshop_qr {
	...
	function clogistik_workshop_qr() {
		$this->TableVar = "logistik_workshop_qr";
		$this->TableName = "logistik_workshop_qr";
		$this->SelectLimit = TRUE;
		$this->actionlink = new cField('logistik_workshop_qr', 'x_actionlink', 'actionlink', "`actionlink`", 201, -1, FALSE);
		$this->fields['actionlink'] =& $this->actionlink;
		$this->qrno = new cField('logistik_workshop_qr', 'x_qrno', 'qrno', "`qrno`", 200, -1, FALSE);
		$this->fields['qrno'] =& $this->qrno;
		$this->idseqno = new cField('logistik_workshop_qr', 'x_idseqno', 'idseqno', "`idseqno`", 3, -1, FALSE);
		$this->fields['idseqno'] =& $this->idseqno;
		$this->prno = new cField('logistik_workshop_qr', 'x_prno', 'prno', "`prno`", 200, -1, FALSE);
		$this->fields['prno'] =& $this->prno;
		$this->kode_pekerjaan = new cField('logistik_workshop_qr', 'x_kode_pekerjaan', 'kode_pekerjaan', "`kode_pekerjaan`", 200, -1, FALSE);
		$this->fields['kode_pekerjaan'] =& $this->kode_pekerjaan;
		$this->partunit = new cField('logistik_workshop_qr', 'x_partunit', 'partunit', "`partunit`", 202, -1, FALSE);
		$this->fields['partunit'] =& $this->partunit;
		$this->kodeworkshop = new cField('logistik_workshop_qr', 'x_kodeworkshop', 'kodeworkshop', "`kodeworkshop`", 200, -1, FALSE);
		$this->fields['kodeworkshop'] =& $this->kodeworkshop;
		$this->tanggal = new cField('logistik_workshop_qr', 'x_tanggal', 'tanggal', "`tanggal`", 133, 7, FALSE);
		$this->fields['tanggal'] =& $this->tanggal;
		$this->periode = new cField('logistik_workshop_qr', 'x_periode', 'periode', "`periode`", 133, 7, FALSE);
		$this->fields['periode'] =& $this->periode;
		$this->gudang = new cField('logistik_workshop_qr', 'x_gudang', 'gudang', "`gudang`", 200, -1, FALSE);
		$this->fields['gudang'] =& $this->gudang;
		$this->notes = new cField('logistik_workshop_qr', 'x_notes', 'notes', "`notes`", 200, -1, FALSE);
		$this->fields['notes'] =& $this->notes;
		$this->createby = new cField('logistik_workshop_qr', 'x_createby', 'createby', "`createby`", 200, -1, FALSE);
		$this->fields['createby'] =& $this->createby;
		$this->createdate = new cField('logistik_workshop_qr', 'x_createdate', 'createdate', "`createdate`", 135, 7, FALSE);
	...
	function HighlightName() {
		return "logistik_workshop_qr_Highlight";
	}
	...
	function SqlSelect() { // Select
		return "SELECT * FROM `logistik_workshop_qr`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `logistik_workshop_qr` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		$SQL = "UPDATE `logistik_workshop_qr` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `logistik_workshop_qr` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[EW_PROJECT_NAME . "_" . $this->TableVar . "_" . EW_TABLE_RETURN_URL] <> "") {
			...
			return "logistik_workshop_qrlist.php";
		}
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("logistik_workshop_qrview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "logistik_workshop_qradd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("logistik_workshop_qredit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("logistik_workshop_qradd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("logistik_workshop_qrdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=logistik_workshop_qr" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		// prno
		$this->prno->ViewValue = $this->prno->CurrentValue;
		$this->prno->CssStyle = "";
		$this->prno->CssClass = "";
		$this->prno->ViewCustomAttributes = "";
