<?php
	...
	if($modebutton=="simpan"){
		...
		//hitung 
		$sql="SELECT room,withppn,withservice,nett,disc FROM trx_additional WHERE kode='$kode'";
		$hsltemp=mysql_query($sql,$db);
		...
		$sql="UPDATE trx_additional SET subtotal1='$subtotal1',ppn='$ppn',subtotal2='$subtotal2',service='$service',grandtotal='$grandtotal',kodebooking='$kodebooking' WHERE kode='$kode'";
		mysql_query($sql,$db);
		?>
		...
		function loaddetailinfo(wintablename,textid,textvalue){
			...
			xmlHttp.onreadystatechange=function() {
				if(xmlHttp.readyState==4) {
					...
					// alert(returnvalue);
					idnamaadditional=textid.replace("kode_add","namaadditional");
					idqty=textid.replace("kode_add","qty");
					idprice=textid.replace("kode_add","price");
					arrreturnvalue=returnvalue.split("|||");
					document.getElementById(idnamaadditional).value=arrreturnvalue[0];
...
<?php
	if(!$_GET["editing"]){//cari kode [adds/yyyymmdd/xxx]
		$tanggal=date("Y-m-d");
		$kode="ADDS/".date("Ymd")."/";
		...
		while($detailvalues=mysql_fetch_assoc($hsldet)){
			...
			?><script language="javascript">document.getElementById("namaadditional[<?php echo $no; ?>]").value="<?php echo $namaadditional; ?>";</script><?php
		}
	}
