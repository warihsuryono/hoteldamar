...
// Define page object
$mst_room_add = new cmst_room_add();
$Page =& $mst_room_add;

// Page init processing
$mst_room_add->Page_Init();

// Page main processing
$mst_room_add->Page_Main();
?>
...
// Create page object
var mst_room_add = new ew_Page("mst_room_add");

// page properties
mst_room_add.PageID = "add"; // page ID
var EW_PAGE_ID = mst_room_add.PageID; // for backward compatibility

// extend page with ValidateForm function
mst_room_add.ValidateForm = function(fobj) {
...
mst_room_add.Form_CustomValidate = 
...
mst_room_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mst_room_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><!-- Add to --><h3><b>Master Room</b></h3><br><br>
...
<?php $mst_room_add->ShowMessage() ?>
<form name="fmst_roomadd" id="fmst_roomadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mst_room_add.ValidateForm(this);">
<p>
...
class cmst_room_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $mst_room->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->LoadFormValues(); // Load form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$mst_room->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $mst_room->CurrentAction = "C"; // Copy Record
		  } else {
		    $mst_room->CurrentAction = "I"; // Display Blank Record
		...
		switch ($mst_room->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$mst_room->SendEmail = TRUE; // Send email on add success
		...
		$mst_room->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $mst_room;
		$mst_room->changeby->CurrentValue = $_SESSION["username"];
		$mst_room->changedate->CurrentValue = ew_CurrentDateTime();
	}
	...
	function RenderRow() {
		...
		} elseif ($mst_room->RowType == EW_ROWTYPE_ADD) { // Add row

			// kode
			$mst_room->kode->EditCustomAttributes = "";
			$mst_room->kode->EditValue = ew_HtmlEncode($mst_room->kode->CurrentValue);
	...
	function AddRow() {
		...
		// Field kode
		$mst_room->kode->SetDbValueDef($mst_room->kode->CurrentValue, "");
		$rsnew['kode'] =& $mst_room->kode->DbValue;
		...
		$bInsertRow = $mst_room->Row_Inserting($rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($mst_room->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
	  ...
	  $curUser = CurrentUserName();
		...
		foreach (array_keys($rs) as $fldname) {
			if ($mst_room->fields[$fldname]->FldDataType <> EW_DATATYPE_BLOB) { // Ignore Blob Field
				$newvalue = ($mst_room->fields[$fldname]->FldDataType == EW_DATATYPE_MEMO) ? "<MEMO>" : $rs[$fldname]; // Memo Field
